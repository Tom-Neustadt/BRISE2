...
class TestTransferLearning:
    ...
    def test_tl_loop(
        ...
        for i in range(min_samples):
            predicted, _, error = send_new_configurations(cs, get_workers)
        ...
        if len(configs) < min_samples:
            print(f"Expected at least {min_samples} configs, got {len(configs)} for: {test_id}")
            raise error or AssertionError(f"Expected at least {min_samples} configs, got {len(configs)} for: {test_id}")
    ...
    def test_mtl_fsl_produces_transferred_best_config(
        ...
        for i in range(min_samples):
            ...
            if predicted is None or len(predicted) <= 0:
                print(f"Failed to get config at iteration {i} for: {test_id}")
                raise error or AssertionError(f"Failed to get config at iteration {i} for: {test_id}")
        ...
        if predicted is None or len(predicted) <= 0:
            print(f"Failed to get best config for: {test_id}")
            raise error or AssertionError(f"Failed to get best config for: {test_id}")
