...
class TestConfigSelectionLoop:
    ...
    def test_cs_produces_configurations(
        ...
        if predicted is None or len(predicted) <= 0:
            raise error or AssertionError(f"Failed to get predicted configurations for: {test_id}")
    ...
    def test_each_iteration_adds_one_config(
        ...
        configs = []
        max_iterations = 25
        min_required_iterations = max_iterations #min_required_iterations = 4
        ...
        if len(configs) < min_required_iterations:
            print(f"Expected at least {min_required_iterations} configs, got {len(configs)} for: {test_id}")
            raise error or AssertionError(f"Expected at least {min_required_iterations} configs, got {len(configs)} for: {test_id}")
